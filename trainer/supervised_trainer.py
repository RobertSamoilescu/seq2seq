        ...
        for epoch in range(start_epoch, n_epochs + 1):
            ...
            for batch in batch_generator:
                ...
                if step % self.print_every == 0 and step_elapsed > self.print_every:
                    ...
                    # tensor board
                    self.writer.add_scalar("train_loss", print_loss_avg, n_iter)

                    # log networks parameters
                    for name, param in model.named_parameters():
                        name = name.replace('.', '/')
                        self.writer.add_histogram(name, param.clone().cpu().data.numpy(), n_iter)
                        self.writer.add_histogram(name + '/grad', param.grad.data.cpu().numpy(), n_iter)

                    if dev_data is not None:
                        dev_loss, accuracy = self.evaluator.evaluate(model, dev_data, self.writer, n_iter)
